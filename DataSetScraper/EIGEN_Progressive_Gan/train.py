...
config.desc = 'pgan-custom-'+sys.argv[1]+'-v2-1gpu-fp16'
config.dataset['tfrecord_dir'] = sys.argv[1]
config.grid['size'] = '256custom'
config.sched['max_minibatch_per_gpu'] = { 512: 8, 1024: 4}
print(config.sched)
config.train['network_snapshot_ticks'] = 100
config.train['total_kimg'] = 500
# print(config.grid)
# 'network_snapshot_ticks': 25, 'total_kimg': 100
...
def setup_snapshot_image_grid(G, training_set,
    size    = '256custom', # '1080p' = to be viewed on 1080p display, '4k' = to be viewed on 4k display.
    ...
    if size == '128custom':
        print('size is '+size)
        gw = np.clip(128 // G.output_shape[3], 3, 32)
        gh = np.clip(128 // G.output_shape[2], 2, 32)
    if size == '256custom':
        print('size is '+size)
        gw = np.clip(256 // G.output_shape[3], 3, 32)
        gh = np.clip(256 // G.output_shape[2], 2, 32)
    if size == '512custom':
        gw = np.clip(512 // G.output_shape[3], 3, 32)
        gh = np.clip(512 // G.output_shape[2], 2, 32)
    if size == '1080p':
...
class TrainingSchedule:
    def __init__(
        ...
        cur_nimg,
        training_set,
        lod_initial_resolution  = 2,        # Image resolution used at the beginning.
        ...
        tick_kimg_dict          = {4: 160, 8:140, 16:120, 32:100, 64:80, 128:60, 256:40, 512:20}): # Resolution-specific overrides.
...
def train_progressive_gan(
    ...
    while cur_nimg < total_kimg * 1000:
        ...
        if cur_nimg >= tick_start_nimg + sched.tick_kimg * 1000 or done:
            ...
            # Save snapshots.
            print('Saving Snap Shot'+ os.path.join(result_subdir, 'network-snapshot-%06d.pkl' % (cur_nimg // 1000)))
